...
if __name__ == '__main__':
    ...
    for d in data_dirs:
        ...
        ssnpkmagif = ssnp.SSNP(epochs=epochs, verbose=verbose, patience=0, opt='adam', bottleneck_activation='linear')
        ...
        ssnpkmlof = ssnp.SSNP(epochs=epochs, verbose=verbose, patience=0, opt='adam', bottleneck_activation='linear')
        y_res_kmlof = cantor_pairing(y_km, y_lof)
        ssnpkmlof.fit(X_train, y_res_kmlof)
        X_ssnpkmlof = ssnpkmlof.transform(X_train)

        ssnpaglof = ssnp.SSNP(epochs=epochs, verbose=verbose, patience=0, opt='adam', bottleneck_activation='linear')
        y_res_aglof = cantor_pairing(y_ag, y_lof)
        ssnpaglof.fit(X_train, y_res_aglof)
        X_ssnpaglof = ssnpaglof.transform(X_train)

        ssnpkmaglof = ssnp.SSNP(epochs=epochs, verbose=verbose, patience=0, opt='adam', bottleneck_activation='linear')
        ...
        results.append(
            (dataset_name, 'SSNP-KMeans-IF',) + compute_all_metrics(X_train, X_ssnpkmif, D_high, D_ssnpkmif, y_train))
